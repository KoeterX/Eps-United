document.addEventListener('DOMContentLoaded', function() {
    ...
    // Register buttons for open days
    const registerButtons = document.querySelectorAll('.register-btn');
    registerButtons.forEach(button => {
        button.addEventListener('click', function() {
            const openDayTitle = this.closest('.open-day-card').querySelector('h3').textContent;
            
            // Scroll to registration form
            const registrationSection = document.getElementById('aanmelden');
            const headerHeight = document.querySelector('header').offsetHeight;
            ...
            // Pre-fill the activity field
            setTimeout(() => {
                const activitySelect = document.getElementById('activity');
                if (activitySelect) {
                    activitySelect.value = 'opendag';
                    showNotification(`Aanmeldformulier geopend voor ${openDayTitle}`, 'info');
                }
            }, 500);
        });
    });
...
function addParticipantToList(registration) {
    ...
    participantItem.innerHTML = `
        <span class="participant-name">${registration.name}</span>
        <span class="participant-activity">${activityNames[registration.activity] || registration.activity}</span>
        <span class="participant-date">${registration.date}</span>
    `;
